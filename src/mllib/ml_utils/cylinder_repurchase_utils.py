...
def cal_cylinder_purchase_qty(orders_df: pd.DataFrame, mapping_combo_qty: dict, assess_date: pd.Timestamp) -> pd.DataFrame:
    ...
    orders_tmp_df = orders_df.copy().assign(
        purchase_qty = np.where(
        (orders_df["product_category_2"]=="鋼瓶") &
        (orders_df["sales_amount"]>0) &
        (orders_df["sales_quantity"]>0),
        orders_df["sales_quantity"], 0),
    )
    tmp_df = orders_tmp_df.query("product_name.str.contains('鋼瓶', na=False)").copy()
    for combo in mapping_combo_qty:
        qty = mapping_combo_qty.get(combo)
        tmp_df.loc[lambda df, c=combo: df["product_name"].str.contains(c), "sales_quantity"] *= qty
        tmp_df.loc[lambda df, c=combo: df["product_name"].str.contains(c), "purchase_qty"] *= qty
